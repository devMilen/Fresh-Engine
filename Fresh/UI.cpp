...
UI::TextBox::TextBox() {}
UI::TextBox::TextBox(const glm::vec2& pos, const std::string& initText, float letterHeight, float letterWidth, float depth)
	: pos(pos), text(initText), h(letterHeight), w(letterWidth)
{
	for (int i = 0; i < text.size(); i++)
		AddLetterArgs(i);

	textBatch = Sprite(vertices, depth, indices, false, texV, texF, fontFilePath, true,
		glm::ortho(0.0f, (float)WINDOW_WIDTH, 0.0f, (float)WINDOW_HEIGHT), VBO_ARGS);
}
UI::TextBox::TextBox(float posX, float posY, const std::string& initText, float letterHeight, float letterWidth, float depth)
