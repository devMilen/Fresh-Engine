...
//	std::cout << "all m_texture ids are valid textures" << std::endl;
...
//	assert(glIsVertexArray(sprite.m_array_buffer.id) && glIsBuffer(sprite.VBO.id) &&
//		glIsBuffer(sprite.EBO.id) && glIsProgram(sprite.m_shader_program.id) && glIsTexture(sprite.m_texture.id));
